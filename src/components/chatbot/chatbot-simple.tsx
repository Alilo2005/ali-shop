    ...
    <>
      ...
      <div className="fixed bottom-6 right-6 z-50">
        <button
          ...
          className={`relative p-4 rounded-full shadow-lg transition-all duration-500 transform hover:scale-110 chat-pulse-glow ${
            isOpen 
        ...
        >
          ...
          {/* Enhanced Tooltip */}
          <div className="absolute bottom-full right-0 mb-3 px-4 py-2 bg-black/90 backdrop-blur-sm text-white text-sm rounded-xl opacity-0 group-hover:opacity-100 transition-all duration-300 whitespace-nowrap shadow-lg">
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-40 w-96 h-[500px] bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-gray-200/50 dark:border-gray-700/50 flex flex-col overflow-hidden chat-slide-in">
          {/* Header */}
          <div className="relative p-4 bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 text-white gradient-animate">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <h3 className="font-bold text-lg bg-white/20 rounded-lg px-2 py-1 backdrop-blur-sm">AI Shopping Assistant</h3>
            ...
            <div className="absolute top-1/2 right-4 w-2 h-2 bg-white/60 rounded-full animate-ping"></div>
            <div className="absolute top-1/3 right-8 w-1 h-1 bg-white/40 rounded-full animate-ping" style={{ animationDelay: '1s' }}></div>
          </div>

          {/* Messages */}
          <div className="flex-1 p-4 overflow-y-auto space-y-4 bg-gradient-to-b from-gray-50/80 to-white/80 dark:from-gray-800/80 dark:to-gray-900/80 backdrop-blur-sm chat-messages">
            {messages.map((message, index) => (
              ...
              >
                <div className={`flex items-start gap-3 max-w-[85%] group`}>
                  {message.sender === 'bot' && (
                    <div className="w-8 h-8 bg-gradient-to-r from-blue-100 to-purple-100 dark:from-blue-900 dark:to-purple-900 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm border-2 border-white dark:border-gray-800">
                      <Bot className="w-4 h-4 text-blue-600 dark:text-blue-400" />
                    </div>
                  )}
                  
                  <div
                    className={`relative p-4 rounded-2xl max-w-xs shadow-sm transition-all duration-300 group-hover:shadow-md ${
                      message.sender === 'user'
                  ...
                  >
                    ...
                    {message.sender === 'bot' && (
                      <div className="absolute -bottom-1 -left-1 w-3 h-3 bg-gradient-to-r from-blue-500 to-purple-500 rounded-full"></div>
                    )}
                  </div>
                  
                  {message.sender === 'user' && (
                    <div className="w-8 h-8 bg-gradient-to-r from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-600 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm border-2 border-white dark:border-gray-800">
            ...
            {messages.length === 1 && (
              <div className="grid grid-cols-2 gap-2 mt-4 animate-in fade-in-0 slide-in-from-bottom-4 duration-700 delay-500">
                {quickReplies.map((reply, index) => (
                  <button
                    key={index}
                    onClick={() => handleQuickReply(reply.value)}
                    className="p-3 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 hover:from-blue-100 hover:to-purple-100 dark:hover:from-blue-900/30 dark:hover:to-purple-900/30 text-gray-700 dark:text-gray-300 rounded-xl text-xs font-medium transition-all duration-300 hover:scale-105 border border-gray-200 dark:border-gray-700"
                  >
                    {reply.text}
                  </button>
                ))}
            ...
            {isTyping && (
              <div className="flex justify-start animate-in fade-in-0 slide-in-from-bottom-4 duration-300">
                <div className="flex items-start gap-3 max-w-[80%]">
                  <div className="w-8 h-8 bg-gradient-to-r from-blue-100 to-purple-100 dark:from-blue-900 dark:to-purple-900 rounded-full flex items-center justify-center flex-shrink-0">
          ...
          {/* Input */}
          <div className="p-4 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border-t border-gray-200/50 dark:border-gray-700/50">
            <div className="flex gap-3 items-end">
              <div className="flex-1 relative">
                <input
                  ...
                  className="w-full p-4 pr-12 border-2 border-gray-300/50 dark:border-gray-600/50 rounded-xl focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/20 dark:bg-gray-800/50 dark:text-white text-sm placeholder-gray-500 transition-all duration-300 backdrop-blur-sm"
                  disabled={isTyping}
                />
            ...
            <div className="flex items-center justify-center gap-2 mt-3">
              ...
              <p className="text-xs text-gray-500 text-center bg-gray-100/50 dark:bg-gray-800/50 px-3 py-1 rounded-full backdrop-blur-sm">
                Powered by AI • Your intelligent shopping companion
              </p>
              <ShoppingBag className="w-3 h-3 text-purple-500 animate-pulse" />
            </div>
          </div>
        </div>
      )}
